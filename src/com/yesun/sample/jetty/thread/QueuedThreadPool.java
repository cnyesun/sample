...
import com.yesun.sample.jetty.thread.ThreadPool.SizedThreadPool;
import com.yesun.sample.jetty.util.component.AbstractLifeCycle;


public class QueuedThreadPool extends AbstractLifeCycle implements SizedThreadPool, Executor
{
     ...
     * 插入job到jobs
     * (non-Javadoc)
     * @see org.eclipse.jetty.util.thread.ThreadPool#dispatch(java.lang.Runnable)
     */
            ...
            if(_jobs.offer(job))
            {
                ...
                if (idle==0 || jobQ>idle)
                {
                    ...
                    if (threads<_maxThreads)
                        startThread(threads);//如果没有空闲线程，且当前线程数小于maxThreads，则继续创建新线程
                }
                return true;
            }
        }
    ...
    /* ------------------------------------------------------------ */
    /**
     * 轮询线程，从jobs中取出job，执行run方法
     */
    private Runnable _runnable = new Runnable()
    {
        public void run()
        {
        	log.debug(Thread.currentThread().getName() + " JOB Runner已启动，用户处理JOBS.");
            ...
            try
            {
                ...
                while (isRunning())
                {
                	System.out.println("QueueThreadPool Job loop");
                    // Job loop
